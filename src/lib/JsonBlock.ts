...
export default class JsonBlock implements Block {
  ...
  constructor (
    ...
    if (blockTimestamp < new Date(2000, 1, 1)) {
      throw new Error(`Expected block time to be later than year 2000, but found ${blockTimestamp}.`)
    }
    // NOTE: previousBlockHash is okay to be empty for block 0
    if (_.isEmpty(previousBlockHash) && height > 0) throw new Error(`invalid previousBlockHash: ${previousBlockHash}`)
