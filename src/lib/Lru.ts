...
export default class Lru<TKey, TValue> {
  ...
  _debugChain (node: LruNode<TKey, TValue>, nextAccessor: (n: LruNode<TKey, TValue>) => LruNode<TKey, TValue>) {
    let keys = []
    while (node) {
      keys.push({
        key: node.key,
  ...
  private onNodeAccessed (node: LruNode<TKey, TValue>): void {
    ...
    // maintain cache size:
    while (this.tail && this.map.size > this.maxSize && this.tail !== this.head) {
